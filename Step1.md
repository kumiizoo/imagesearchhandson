...
1. *ユーザのエイリアスの設定* タブを選択し、 *RAM ユーザーログイン* をメモします
  * *RAM ユーザーログイン* 例 : `http://signin-intl.aliyun.com/ <<アカウントID>>/login.htm`
1. 右上の絵文字アイコンを押下し、*サインアウト* を押下します
1. 3.でメモした *RAM ユーザーログイン* にアクセスします
1. 次の通り入力し、ログインします
  * *ユーザー名* : `AdminUser@<<アカウントID>>`
  * *パスワード* : `任意の文字列`
...
1. 右上の *新規ユーザー* ボタンを押下します
1. 次の通り入力します
  * *ログイン名* : `ImageSearchHandsOnUser`
  * *このユーザーの AccessKey を自動生成* : **チェックをつける**
1. *OK* ボタンを押下します
